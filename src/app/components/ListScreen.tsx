  ...
  return (
    <div className="min-h-screen bg-[#F9FBE7]">
      ...
      <div className="bg-[#2E7D32] text-white px-4 py-4">
        ...
        <p className="text-[#A5D6A7] text-xs mt-1">{vendors.filter(v => v.certified).length} certificados · {vendors.length} en total</p>
      </div>
      ...
      <div className="px-3 py-3 space-y-3">
        {sorted.map(v => (
          <button key={v.id} onClick={() => navigate(`/seller/${v.id}`)} className="w-full overflow-hidden rounded-[28px] border border-gray-100 bg-white shadow-sm transition hover:shadow-md text-left">
            <div className="grid gap-4 md:grid-cols-[100px_1fr] p-4">
              <img src={v.imageUrl} alt={`${v.name} preview`} className="h-24 w-full rounded-3xl object-cover md:h-24 md:w-24" />
              <div className="flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between gap-3 mb-2">
                    <h3 className="font-semibold text-[#1B5E20] text-sm truncate">{v.name}</h3>
                    {v.certified
                      ? <span className="rounded-full bg-[#E8F5E9] px-2 py-1 text-[11px] font-semibold text-[#2E7D32]">✓ Seca</span>
                      : <span className="rounded-full bg-orange-50 px-2 py-1 text-[11px] font-semibold text-orange-600">Sin medición</span>
                    }
                  </div>
                ...
                <div className="mt-4 flex flex-wrap items-center justify-between gap-2 text-xs text-gray-500">
                  <div>
                    <span className="font-semibold text-gray-900">${v.price.toLocaleString('es-CL')}</span>
